            ...
            items: [
                {
                    ...
                    icon: 'pi pi-refresh',
                    command: () => {
                        toast.add({ severity: 'success', summary: 'Updated', detail: `${course_code}`, life: 3000 });
                    }
                },
                {
                    label: 'Delete',
                    icon: 'pi pi-times',
                    command: () => {
                        toast.add({ severity: 'warn', summary: 'Delete', detail: `${course_code}`, life: 3000 });
                    }
                }
            ]
        }
...
function dropCourse(code) {
    ...
    axios.post(API_URL + "/course-action/drop-course", {course_code: code}, config).then((response) => {
        console.log(response.data)
        toast.add({ severity: 'success', summary: 'Sucesss', detail: response.data.message, life: 3000 })
        fetchData()
    }).catch(error=>{
        console.log(error)
    })
...
function fetchData() {
    ...
    axios.get(API_URL + "/course/registered-courses", config).then((response) => {
...
<template>
    ...
    <div class="card p-3 mt-3">
        <CourseCard v-if="courses.length > 0" v-for="(course, index) in courses" :key="index" :name="course.name"
            :code="course.code" :credits="course.credits" :startDate="course.start_date" :endDate="course.end_date"
            :schedules="course.schedules" :details="false">
            <template #menu>
                <CourseCardMenu :coursecode="course.code" @detail="watchCourse" @drop="dropCourse"></CourseCardMenu>
            </template>
        </CourseCard>
