                ...
                <div class="w-full surface-card py-8 px-5 sm:px-8" style="border-radius: 53px">
                    ...
                    <div>
                        <div class="field">
                            ...
                            <div class="error-msg" v-if="error.username">{{errMsg.username}}</div>
                        </div>
                        <div class="field">
                        ...
                        <div class="field">
                            <label for="firstName" class="block text-900 text-xl font-medium ">Firstname</label>
                            <InputText id="firstName" type="text" placeholder="Firstname" class="w-full "
                                style="padding: 1rem" v-model="firstName"
                                :class="{ 'p-invalid': error.firstName || !registerSuccessful }" />
                            <div class="error-msg" v-if="error.firstName">{{errMsg.firstName}}</div>
                        </div>
                        <div class="field">
                            <label for="lastName" class="block text-900 text-xl font-medium ">Lastname</label>
                            <InputText id="lastName" type="text" placeholder="Lastname" class="w-full "
                                style="padding: 1rem" v-model="lastName"
                                :class="{ 'p-invalid': error.lastName || !registerSuccessful }" />
                            <div class="error-msg" v-if="error.lastName">{{errMsg.lastName}}</div>
                        </div>
                        ...
                        <div class="field">
                            ...
                            <div class="error-msg" v-if="error.password">{{errMsg.password}}</div>
                        </div>
                        <div class="field">
                            ...
                            <Password id="password2" v-model="confirmPassword" placeholder="Confirm Password"
                                ...
                                :class="{ 'p-invalid': error.confirmPassword || !registerSuccessful }">
                            </Password>
                            <div class="error-msg" v-if="error.confirmPassword">{{errMsg.confirmPassword}}</div>
                        </div>
...
const register = async () => {
    if (validate())
        try {
            const res = await authStore.register({ username: username.value, email: email.value, password: password.value, code: code.value, first_name: firstName.value, last_name: lastName.value });
            console.log(res);
...
const validate = () => {
    ...
    (error.username = (username.value === "")) && (errMsg.username = "Username is empty");
    (error.email = (!validateEmail(email.value))) && (errMsg.email = "Email is empty or in wrong format");
    (error.password = (password.value === "")) && (errMsg.password = "Password is empty");
    (error.confirmPassword = (confirmPassword.value === "")) && (errMsg.confirmPassword = "Confirm password is empty");
    (error.firstName = firstName.value === "") && (errMsg.firstName = "Firstname is empty");
    (error.lastName = lastName.value === "") && (errMsg.lastName = "Lastname is empty");
    if(confirmPassword.value!== password.value){
        error.confirmPassword = true;
        errMsg.confirmPassword = "Confirm password does not match";
    }
    return !(error.username || error.email || error.password || error.confirmPassword || error.firstName || error.lastName);
}
