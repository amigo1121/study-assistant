...
const validate= () => {
    ...
    if (allDay.value) {
        valid.value = valid.value && moment(startDate.value).isValid() && moment(endDate.value).isValid() && moment(startDate.value).diff(moment(endDate.value), 'days') < 0
    }
    else {
        valid.value = valid.value && moment(startDate.value).isValid() && moment(endDate.value).isValid() && moment(startDate.value).diff(moment(endDate.value), 'days') <= 0
        if (moment(startDate.value).diff(moment(endDate.value), 'days') === 0) {
            valid.value = valid.value && moment(startTime.value).diff(moment(endTime.value), 'seconds') < 0
        }
    }
    return valid.value
}
...
const remove =  async () =>{
    try {
        if (confirm(`Are you sure you want to delete the event '${title.value}'`))
        {
            await eventStore.deleteEvent(props.id)
            emit('submit',"Delete event")
        }
    }
    catch(error)
    {
...
const submit = async () => {
    if (validate()) {
        ...
        if (allDay.value) {
            ...
            inputEvent.start = moment(startDate.value).hour(startTimeHour).minute(startTimeMinute).format();
            inputEvent.end = moment(endDate.value).hour(endTimeHour).minute(endTimeMinute).format();
        }
        try {
            await eventStore.updateEvent(inputEvent);
            emit('submit',"Modify event")
        } catch (error) {
            console.log(error)
        }
    }
...
<template>
    <div class="p-fluid formgrid grid">
        ...
        <div class="flex col-12 gap-2 flex-nowrap justify-content-end">
            <Button class="flex max-w-min" severity="danger" label="Delete" @click="remove" />
